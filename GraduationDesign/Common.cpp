...
{
	if(src==0) return;
	if(!dst) dst = _malloc(_sizeof(src));
...
int _strcpy(char *&szDst,char *szSrc)
{
	DWORD len = strlen(szSrc);

	szDst = new char[len+1];
	strcpy(szDst,szSrc);
	return len;
}

void _writebytes(DAT_TYPE *pBytes,FILE *fp)
{
	fwrite(pBytes,1,_sizeof(pBytes),fp);
}

DWORD _fread(DAT_TYPE *&dst,DWORD size,DWORD count,FILE *fp)
{
	if(count == 0 || size==0)
	{
		dst = 0;
	...
	dst = _malloc(count*size);
	return fread(dst,size,count,fp);
}

DWORD _fwrite(DAT_TYPE *src,FILE *fp)
{
	if(_sizeof(src)==0) return 0;
	return fwrite(src,1,_sizeof(src),fp);
}
